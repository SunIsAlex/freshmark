...
export class BuildWorkerPool {
  ...
  #createWorker() {
    ...
    worker.on("exit", (code) => {
      if (!this.closed) this.#fail(new Error(`Build worker exited unexpectedly with code ${code}`));
    });
  ...
  async close() {
    ...
    this.closed = true;
    this.#rejectTasks(this.failure || new Error("Build worker pool closed before completing its work"));
    this.closing = Promise.all(this.workers.map(({ worker }) => worker.terminate()));
    await this.closing;
  }
}
