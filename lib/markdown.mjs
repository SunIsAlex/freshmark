...
export function searchTextFromMarkdown(markdown) {
  ...
  const visit = (tokens = []) => {
    for (const token of tokens) {
      ...
      else if (token.type === "text" || token.type === "code_inline" || token.type === "code_block" || token.type === "fence" || token.type === "html_inline" || token.type === "html_block" || token.type === "image") append(token.content);
