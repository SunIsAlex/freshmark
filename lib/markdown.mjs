...
function searchableMathText(source) {
...
export function searchTextFromMarkdown(markdown) {
  ...
  const append = (value) => {
    const text = String(value || "")
      .replace(/FRESHMARKMATH(\d+)END/g, (_, index) => ` ${searchableMathText(math[Number(index)]?.source)} `)
      .replace(/<!--[\s\S]*?-->/g, " ")
      .replace(/<[^>]+>/g, " ");
  ...
  return searchableMathText(parts.join(" "))
