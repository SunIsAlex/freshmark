...
import { renderMathHtml, renderMathmlFallback } from "./math-html.mjs";
import { searchableLatexText } from "./search-text.mjs";
...
export function searchTextFromMarkdown(markdown) {
  const { protectedMarkdown, math } = protectMath(String(markdown || ""));
  const md = new MarkdownIt({ html: true, linkify: true, typographer: false });
  const parts = [];
  const append = (value) => {
    const text = String(value || "")
      .replace(/FRESHMARKMATH(\d+)END/g, (_, index) => ` ${searchableLatexText(math[Number(index)]?.source)} `)
      .replace(/<!--[\s\S]*?-->/g, " ")
      .replace(/<[^>]+>/g, " ");
  ...
  const visit = (tokens = []) => {
    for (const token of tokens) {
      if (token.type === "inline") visit(token.children);
      else if (token.type === "text" || token.type === "code_inline" || token.type === "code_block" || token.type === "fence" || token.type === "html_inline" || token.type === "html_block") append(token.content);
      else if (token.type === "softbreak" || token.type === "hardbreak") parts.push(" ");
    }
  };
  visit(md.parse(protectedMarkdown, {}));
  return searchableLatexText(parts.join(" "))
...
function protectMath(markdown) {
  ...
  const processText = (text) => text
    .replace(/(\$[^$\n]+)\$\$(?=\S)/g, "$1$ $")
    .replace(/\$([^$]*?\\begin\{([^}]+)\}[\s\S]*?\\end\{\2\}[^$]*)\$/g, (_, source) => `$${source.replace(/\s*\n\s*/g, " ")}$`)
...
function renderMathSource(source, { display = false, fallback = "" } = {}) {
  const className = display ? "math-expression math-display" : "math-expression math-inline";
  const label = fallback ? ` aria-label="${escapeHtml(source)}"` : "";
  return `<span class="${className}"${label} data-math-source="${escapeHtml(source)}"${display ? " data-math-display" : ""}>${fallback}</span>`;
}
...
async function replaceMathTokens(html, math, mathOutput) {
  ...
  const replacements = await Promise.all(matches.map(async (match) => {
    ...
    if (mathOutput === "mathml") {
      return renderMathHtml(expression.source, { display: expression.display, output: "mathml" });
    }
    if (mathOutput === "hybrid") {
      return renderMathSource(expression.source, {
        display: expression.display,
        fallback: renderMathmlFallback(expression.source, { display: expression.display }),
      });
    }
    if (mathOutput === "source") {
      return renderMathSource(expression.source, { display: expression.display });
    }
    return renderMathHtml(expression.source, { display: expression.display });
  }));
  let rendered = "";
  let cursor = 0;
...
export async function renderSummary(markdown, { links = true, mathOutput = "html" } = {}) {
...
export async function renderMarkdown(markdown, { mathOutput = "html" } = {}) {
  const headings = [];
  const md = new MarkdownIt({ html: true, linkify: true, typographer: false });
