...
export function normalizePdfOrderedLists(html) {
  return html.replace(/<ol start="(\d+)">/g, (_tag, start) => {
    const counterStart = Number.parseInt(start, 10) - 1;
    return `<ol start="${start}" style="counter-reset:list-item ${counterStart}">`;
  });
}
...
export async function buildPostPdfs(posts, {
  ...
  const renderer = configuredRenderer || await findPdfRenderer({ root, environment });
  ...
  const documents = await Promise.all(posts.map(async (post, index) => {
    ...
    const articleHtml = normalizePdfOrderedLists(post.pdfHtml);
