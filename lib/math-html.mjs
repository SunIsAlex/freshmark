...
export function renderMathmlFallback(source, { display = false } = {}) {
  const key = `fallback\0${display ? "display" : "inline"}\0${source}`;
  if (!rendered.has(key)) {
    rendered.set(key, katex.renderToString(source, { ...katexOptions, displayMode: display, output: "mathml" }));
  }
  return rendered.get(key);
}
