...
export async function enhanceResponsiveImages(posts, {
  ...
  const enhanceHtml = async (post, target) => {
    const sourceHtml = post[target];
    const imageTags = [...sourceHtml.matchAll(/<img\b[^>]*>/gi)];
    if (!imageTags.length) return;
    let html = "";
    let cursor = 0;
    for (const match of imageTags) {
      html += sourceHtml.slice(cursor, match.index);
    ...
    post[target] = html + sourceHtml.slice(cursor);
  };

  const enhancePost = async (post) => {
    await enhanceHtml(post, "html");
    await enhanceHtml(post, "spaHtml");
  };
