...
const viewsEnabled = netlifyFunctionsEnabled();
const viewsEndpoint = "/api/views";
const commentsEnabled = resolveCommentsEnabled();
const commentsAuthEnabled = resolveCommentsAuthEnabled();
const commentsListEndpoint = "/api/comments";
const commentsSubmitEndpoint = "/api/comments/submit";
const commentsAuthEndpoints = {
...
function footer(locale) {
  const messages = locales[locale];
  const views = viewsEnabled ? `<span class="view-count" data-site-view-count hidden>${escapeHtml(messages.siteViews)} <strong data-site-views></strong></span>` : "";
  return `<footer class="container footer"><div class="footer-copy"><span>© ${currentYear} ${escapeHtml(config.title)}. ${escapeHtml(messages.footerNote)}</span>${views}</div><div class="footer-links"><a href="${localeHref(locale, "/about/")}">${escapeHtml(messages.about)}</a><a href="${localeHref(locale, "/rss.xml")}">RSS</a><a href="#">${escapeHtml(messages.top)}</a></div></footer>`;
}
...
async function homePage(locale, posts, { mathOutput = "html", fragment = false } = {}) {
  ...
  const cardMathOutput = mathOutput === "source" ? "source" : "text";
  const cards = (await Promise.all(posts.filter((post) => post !== featured).map(async (post) => `<a class="post-card" href="${localeHref(locale, `/posts/${post.slug}/`)}" data-post-card data-tags="${escapeHtml(post.categories.join("|"))}"><time class="post-date" datetime="${post.date}">${formatDate(locale, post.date)}</time><div><h3>${escapeHtml(post.title)}</h3><p>${await renderSummary(post.summary, { links: false, mathOutput: cardMathOutput })}</p></div><span class="post-arrow" aria-hidden="true">↗</span></a>`))).join("");
...
async function postPage(post, { mathOutput = "html", fragment = false } = {}) {
  ...
  const views = viewsEnabled ? `<span class="view-count" data-article-view-count hidden>${escapeHtml(messages.articleViews)} <strong data-article-views></strong></span>` : "";
  const authPanel = commentsAuthEnabled
    ? `<div class="comment-auth" data-comment-auth><p class="comment-auth-status" data-auth-loading>${escapeHtml(messages.authLoading)}</p><div data-auth-guest hidden><form class="comment-auth-form" data-auth-login><h3>${escapeHtml(messages.authLoginTitle)}</h3><label>${escapeHtml(messages.authEmail)}<input name="email" type="email" required maxlength="254" autocomplete="email"></label><label>${escapeHtml(messages.authPassword)}<input name="password" type="password" required maxlength="128" autocomplete="current-password"></label><div class="comment-form-foot"><p class="comment-form-status" data-auth-login-status role="status"></p><button type="submit">${escapeHtml(messages.authLogin)}</button></div></form><details class="comment-register"><summary>${escapeHtml(messages.authRegisterTitle)}</summary><form class="comment-auth-form" data-auth-register><label>${escapeHtml(messages.commentName)}<input name="name" required maxlength="40" autocomplete="name"></label><label>${escapeHtml(messages.authEmail)}<input name="email" type="email" required maxlength="254" autocomplete="email"></label><label>${escapeHtml(messages.authPassword)}<input name="password" type="password" required minlength="8" maxlength="128" autocomplete="new-password"></label><p class="comment-auth-hint">${escapeHtml(messages.authPasswordHint)}</p><div class="comment-form-foot"><p class="comment-form-status" data-auth-register-status role="status"></p><button type="submit">${escapeHtml(messages.authRegister)}</button></div></form></details></div><form class="comment-auth-form" data-auth-verify hidden><p>${escapeHtml(messages.authCodeSent)}</p><label>${escapeHtml(messages.authVerificationCode)}<input name="code" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" minlength="6" maxlength="6" required></label><div class="comment-form-foot"><p class="comment-form-status" data-auth-verify-status role="status"></p><button type="submit">${escapeHtml(messages.authVerify)}</button></div></form><div class="comment-auth-user" data-auth-user hidden><p>${escapeHtml(messages.authLoggedInAs)} <strong data-auth-user-name></strong> <span data-auth-user-email></span></p><button type="button" data-auth-logout>${escapeHtml(messages.authLogout)}</button></div></div>`
    : "";
  const identityFields = commentsAuthEnabled
    ? ""
