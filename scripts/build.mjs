...
import {
  commentsEmailVerificationEnabled as resolveCommentsEmailVerificationEnabled,
...
const commentsEmailVerificationEnabled = resolveCommentsEmailVerificationEnabled();
const commentsListEndpoint = "/api/comments";
const commentsSubmitEndpoint = "/api/comments/submit";
const commentsVerifyEndpoint = "/api/comments/verify";
let assetVersion = "";
...
async function postPage(post, { mathOutput = "html", fragment = false } = {}) {
  ...
  const verificationForm = commentsEmailVerificationEnabled
    ? `<div class="comment-verification" data-comment-verification hidden><p>${escapeHtml(messages.commentVerificationSent)}</p><label>${escapeHtml(messages.commentVerificationCode)}<input name="verification-code" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" minlength="6" maxlength="6"></label><div class="comment-form-foot"><p class="comment-form-status" data-comment-verification-status role="status"></p><button type="button" data-comment-verify>${escapeHtml(messages.verifyComment)}</button></div></div>`
    : "";
  const commentsIntro = commentsEmailVerificationEnabled ? messages.commentsVerificationIntro : messages.commentsIntro;
  const commentEmail = commentsEmailVerificationEnabled ? messages.commentVerifiedEmail : messages.commentEmail;
  const comments = commentsEnabled ? `<section class="comments container" data-comments data-comments-path="${escapeHtml(localizedPath(post.locale, `/posts/${post.slug}/`))}"><div class="comments-head"><div><p class="eyebrow">${escapeHtml(messages.commentsTitle)}</p><h2 data-comments-title>${escapeHtml(messages.commentsTitle)}</h2></div><p>${escapeHtml(commentsIntro)}</p></div><div class="comment-list" data-comment-list aria-live="polite"><p class="comment-state" data-comment-state>${escapeHtml(messages.commentsLoading)}</p></div><button class="comment-more" type="button" data-comments-more hidden>${escapeHtml(messages.loadOlderComments)}</button><form class="comment-form" data-comment-form><div data-comment-compose><div class="comment-fields"><label>${escapeHtml(messages.commentName)}<input name="name" required maxlength="40" autocomplete="name"></label><label>${escapeHtml(commentEmail)}<input name="email" type="email" maxlength="254" autocomplete="email"${commentsEmailVerificationEnabled ? " required" : ""}></label></div><label>${escapeHtml(messages.commentBody)}<textarea name="body" required minlength="2" maxlength="2000" rows="5" placeholder="${escapeHtml(messages.commentPlaceholder)}"></textarea></label><label class="comment-honeypot" aria-hidden="true">Website<input name="website" tabindex="-1" autocomplete="off"></label><div class="comment-form-foot"><p class="comment-form-status" data-comment-form-status role="status"></p><button type="submit" data-comment-submit>${escapeHtml(messages.submitComment)}</button></div></div>${verificationForm}</form></section>` : "";
