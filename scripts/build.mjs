...
const navigationResponse=async(request,event)=>{const cached=await caches.match(request);const fresh=fetch(request).then((response)=>cacheResponse(request,response)).catch(()=>null);if(cached){event.waitUntil(fresh);return cached}const response=await fresh;if(response)return response;const path=new URL(request.url).pathname.slice(BASE_PATH.length);const fallback=LOCALIZED_NOT_FOUND.find(([prefix])=>path.startsWith(prefix))?.[1]||"/404.html";return caches.match(at(fallback))};
